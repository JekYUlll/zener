...
namespace zws {
    ...
    void Logger::Init() {
        ...
        if (sInitialized) {
            if (sSpdLogger) {
                #ifdef DEBUG
                sSpdLogger->warn("Logger already initialized at {}:{}, current call from {}:{}",
        ...
        auto new_logger = spdlog::stdout_color_mt<spdlog::async_factory>("async_logger");
    ...
    void Logger::log(spdlog::source_loc loc, spdlog::level::level_enum lvl, const spdlog::memory_buf_t *buffer) {
        auto logger = std::atomic_load(&sSpdLogger);
        if (logger) {
            logger->log(loc, lvl, spdlog::string_view_t(buffer->data(), buffer->size()));
        }
    }
